...
categorys = ['intersectional']
csv_result_file = '../output/intersectional_results_overview.csv'
...
for path in paths:
    ...
    paragraphs = [[x for x in lines_file if str(x[0]).startswith("intersectional")
                   and any(y == str(x[8]).split('+')[0] for y in list1)
                   and any(y == str(x[8]).split('+')[1] for y in list2)]]#intersectional
    if nb_line == 0:
