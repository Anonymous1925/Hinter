...
def fct_modification(fct_replacement, word_list, replacement_list=[]):
    ...
    for i_word in range(len(word_list)):
        ...
        if word_error > 0:
            print("> Found {} occurrences of \'{}\' for {} error ({} total)."
                  .format(total_word_occurrence, word, word_error, total_error))
    return l_replaced_occurrences, l_changed_cases, l_changed_prediction_details, l_more_label_counter, l_less_label_counter
...
def fct_intersectional(word_list_1, replacement_list_1, word_list_2, replacement_list_2):
    ...
    for i in range(complete_dataset_size):
        ...
        if case_error > 0:
            ...
            print(">>Case {}/{} modified {} time(s) for {} error(s) ".format(i, complete_dataset_size, case_mutant,
                                                                           case_error))
    print("Total of {} mutants and {} errors.".format(l_changed_cases, total_error))
    return 0, l_changed_cases, l_changed_prediction_details, l_more_label_counter, l_less_label_counter
...
if method_name == 'replacement':
    replaced_occurrences, changed_cases, changed_prediction_details, more_label_counter, \
    less_label_counter = fct_replacement(new_word_list_A, new_word_list_B)

if is_intersectional:
    replaced_occurrences, changed_cases, changed_prediction_details, more_label_counter, \
    less_label_counter = fct_intersectional(new_word_list_A, new_word_list_B, inter_list_A, inter_list_B)
